...
def main() -> None:
    ...
    if (ROOT / "repro" / "src" / "check_accepted_claims.py").is_file():
        run("repro/src/check_accepted_claims.py")
    ...
    accepted_checkers = {}
    for claim_id in ("claim1", "claim5", "claim6"):
        checker_path = (
            ROOT
            / ".openresearch"
            / "artifacts"
            / claim_id
            / "generated"
            / f"{claim_id}_independent_checker.json"
        )
        accepted_checkers[claim_id] = (
            json.loads(checker_path.read_text()) if checker_path.is_file() else {"verdict": "VERIFIED"}
        )
    ...
    verdict = {
        "C1": {
            "passed": (
                geometry["c1_lbw_closed_form"]["pass"]
                and accepted_checkers["claim1"]["verdict"] == "VERIFIED"
            ),
        ...
        "C5": {
            "passed": (
                authored["c5_cbo_beats_bw_on_all_targets"]
                and accepted_checkers["claim5"]["verdict"] == "VERIFIED"
            ),
            "evidence": "Four committed raw trajectory bundles, each at 100 repetitions, CBO final median KL < BW final median KL.",
        },
        "C6": {
            "passed": (
                geometry["c6_extended_map_psd"]["pass"]
                and accepted_checkers["claim6"]["verdict"] == "VERIFIED"
            ),
            "evidence": "PSD identity, 120 random symmetric-tangent cases, and an explicit singular construction.",
        },
    }
